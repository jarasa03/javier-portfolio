...
@php
    $hasHeader = $title || $subtitle || ($ctaLabel && $ctaUrl);
@endphp

<div @class(['surface', 'relative', 'p-6', 'lg:p-8', $surfaceClass]) data-carousel>
    @if ($hasHeader)
    ...
    @if (count($items) > 0)
        <button type="button" data-carousel-prev class="absolute left-3 z-10 -translate-y-1/2 cursor-pointer rounded-full border border-brand-soft bg-white/95 p-3 text-brand transition hover:border-brand hover:bg-brand-soft/40 {{ $hasHeader ? 'top-[calc(50%+1.75rem)]' : 'top-1/2' }}" aria-label="Anterior">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 18l-6-6 6-6" />
            </svg>
        </button>
        <button type="button" data-carousel-next class="absolute right-3 z-10 -translate-y-1/2 cursor-pointer rounded-full border border-brand-soft bg-white/95 p-3 text-brand transition hover:border-brand hover:bg-brand-soft/40 {{ $hasHeader ? 'top-[calc(50%+1.75rem)]' : 'top-1/2' }}" aria-label="Siguiente">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 6l6 6-6 6" />
            </svg>
        </button>

        <div class="{{ $hasHeader ? 'mt-6' : 'mt-0' }} overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white p-3">
